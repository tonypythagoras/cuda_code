...
__global__ void bsfCSRKernel(GRAPH *graph, int *levels, int *again, int currentLevel){
...
//copy memory from host to device
    ...
    // starting from a particular vertext, it searches for its neighbors and assign values to them.
     ...
     while(again_h==1){
        ...
        bsfCSRKernel <<< numberOfBlocks, threadPerBlock >>> (myGRAPH_D, level_d, again_d, currentLevel);
