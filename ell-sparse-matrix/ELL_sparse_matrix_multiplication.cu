#include "cuda_runtime.h"
#include "device_launch_parameters.h"
#include "iostream"
#include <stdio.h>
#include <stdlib.h>

unsigned const int N=8;
unsigned const int P = 4;
unsigned const int NUM_BLOCK=4;
unsigned const int THREAD_PER_BLOCK=4;


struct ELL {  
  int numRow;
  int numCol;
  int maxNNPerRow;
  int *numberOfNonPerRow;           
  int *col;
  int *values;
};




// This performs a sparse matrix multiplication using ell format
__global__ void ellSparseMatrixKernel(ELL *ell, int *invector, int *outvector){
   int row = blockIdx.x * blockDim.x + threadIdx.x;
   if(row < (ell->numRow)){
    int sum=0;
    for( int iter = 0; iter< ell->numberOfNonPerRow[row]; iter++){
    int i= iter * ell->numRow + row;
    int col = ell->col[i];
    int value = ell->values[i];
     sum +=invector[col]*value;
   }
   outvector[row]=sum;
   }
      

}






// initializes data
void initArray(int *rowPtr, int *col, int *values, int *vect){
  values[0]=1;
  values[1]=5;
  values[2]=2;
  values[3]=6;
  values[4]=7;
  values[5]=3;
  values[6]=8;
  values[7]=-1;
  values[8]=-1;
  values[9]=9;
  values[10]=-1;
  values[11]=-1;



  col[0]=0;
  col[1]=0;
  col[2]=1;
  col[3]=3;
  col[4]=1;
  col[5]=2;
  col[6]=2;
  col[7]=-1;
  col[8]=-1;
  col[9]=3;
  col[10]=-1;
  col[11]=-1;




  rowPtr[0]=2;
  rowPtr[1]=3;
  rowPtr[2]=2;
  rowPtr[3]=1;
 
  vect[0]=2;
  vect[1]=1;
  vect[2]=3;
  vect[3]=1;





    
}


int main(){
size_t size= N * sizeof(int);
size_t size_row= 5 * sizeof(int);
size_t size_v= 12 * sizeof(int);
size_t size_non= 4 * sizeof(int);



size_t size_p= P * sizeof(int);
size_t coo_size= (3 * size) + 1;

int *invector_h;
int *outvector_h;
int *row_h;
int *col_h;
int *values_h;
// allocate memories on the host
row_h=(int*)malloc(size_non);
col_h=(int*)malloc(size_v);
values_h=(int*)malloc(size_v);
invector_h=(int*)malloc(size_p);
outvector_h=(int*)malloc(size_p);



initArray(row_h, col_h, values_h,invector_h);
     ELL myEll;


     myEll.numRow=4;
     myEll.numberOfNonPerRow=row_h;
     myEll.col=col_h;
     myEll.values=values_h;

     ELL *myELL_D;




int *invector_d;
int *outvector_d;




cudaError_t err;
// allocates memories on the device

err = cudaMalloc((void**)&invector_d,size_p);
 if(err !=cudaSuccess){
               fprintf(stderr, "cudaMalloc d_A error: %s\n", cudaGetErrorString(err)); exit(EXIT_FAILURE); 
       }

err = cudaMalloc((void**)&outvector_d,size_p);
 if(err !=cudaSuccess){
               fprintf(stderr, "cudaMalloc d_A error: %s\n", cudaGetErrorString(err)); exit(EXIT_FAILURE); 
       }


err=cudaMalloc((void**)&myELL_D, sizeof(ELL));
 if(err !=cudaSuccess){
               fprintf(stderr, "cudaMalloc d_A error: %s\n", cudaGetErrorString(err)); exit(EXIT_FAILURE); 
       }


  

//copy memory from host to device
    cudaMemcpy(myELL_D,&myEll,sizeof(ELL),cudaMemcpyHostToDevice);

    cudaMemcpy(invector_d,invector_h,size_p,cudaMemcpyHostToDevice);

     


     
     // inilizes thread and threadblocks
     dim3 threadPerBlock(8);
     dim3 numberOfBlocks(1);

    // launches kernel for execution
     ellSparseMatrixKernel <<< numberOfBlocks, threadPerBlock >>> (myELL_D, invector_d, outvector_d);

    // copy memories fro device to host
     cudaMemcpy(outvector_h, outvector_d,size_p,cudaMemcpyDeviceToHost);
         

   // output display
 for (int i=0;i<P;i++){
    std::cout<< outvector_h[i]<< std::endl;
   }

// free some device memories
cudaFree(myELL_D);
cudaFree(outvector_d);
cudaFree(invector_d);



// free some host memories
delete[] outvector_h;
delete[] invector_h;
delete[] col_h;
delete[] row_h;
delete[] values_h;

    return 0;
}